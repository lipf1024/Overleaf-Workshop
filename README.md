...
在源代码管理视图中，**Incoming** 和 **Outgoing** 标题右侧分别提供“拉取全部传入文件”和“上传全部传出文件”按钮。按钮只处理点击时该组中的文件，不会同步其他分组；空组或由另一窗口管理的副本不显示按钮。执行中显示进度并阻止重复点击，未保存文件、冲突和暂停项仍需先处理。若文件状态已经改变，也不会自动改为反方向同步。

#### 同步忽略文件

被忽略的文件及整个被忽略的目录显示为灰色。已有同步基线的文件额外显示 `Ⅱ`，表示已停止同步，两端内容可能不同；从未同步的忽略项不显示徽标。灰色不会从子文件向父目录传播。右键“Local Replica: 编辑同步忽略规则”可编辑规则，保存后重新检查状态。

本地副本根目录的 **`.overleafignore`** 是同步忽略规则的来源。首次使用时写入默认规则；旧的自定义忽略列表会作为初始内容迁移，已有文件不会被覆盖。直接打开远程项目不会创建或使用此文件。插件不读取 `.gitignore` 或 Git 全局忽略配置。

- 在资源管理器选中文件或文件夹（支持多选），右键 **Local Replica: 排除同步**，会将项目相对路径追加到 `.overleafignore` 并保存。文件夹规则覆盖其全部内容。
- 运行 **Overleaf Workshop: Local Replica: 编辑同步忽略规则**，或从 Local Replica 的配置菜单打开该文件，可直接编辑规则。保存后重新检查待同步项；需要上传或拉取的内容可在源代码管理视图中确认。
- 使用 gitignore 语法：每行一条规则，`#` 表示注释，`*` / `**` 表示通配，`/` 开头限定项目根目录，`/` 结尾限定目录，`!` 表示取消忽略。被忽略的父目录需要先取消忽略，子文件的例外才能生效。旧配置采用 glob，含花括号或扩展 glob 的自定义规则迁移后需要改写为多行 gitignore 规则。
- 默认忽略隐藏文件、隐藏目录以及常见 LaTeX 编译产物。`.overleafignore` 和插件内部 `.overleaf` 数据始终不参与同步，不能通过 `!` 解除。
- 忽略不会删除本地或远端已有文件；移除规则后重新比较两端，保留正常冲突保护。已经发出的同步请求无法撤回。
- 误删 `.overleafignore` 时，本次会话保留最后加载的规则；通过“Local Replica: 编辑同步忽略规则”可恢复文件。若在文件缺失时重新打开项目，则重新初始化规则。
- `.overleafignore` 可以提交到 Git，方便共享；插件不会替你修改 Git 的忽略规则。
