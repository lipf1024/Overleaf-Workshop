...
0.16.23 补充：目录类型从本地 stat 与已加载远端项目树判断，不读取目录正文。远端目录创建/移动事件会枚举文件路径，仅协调对应子树；普通单文件保存没有新增全树扫描。误生成的目录错误记录通过启动扫描或同步入口清理，有恢复记录时保留。

0.16.22 补充：保存编译不再执行全项目队列等待或检查所有待上传记录；只等待本轮路径的同步与对应 OT 确认。忙时合并请求保留所有涉及的路径；后续保存不延长已捕获的 OT 确认屏障。其余待同步文件仅提示，不新增请求/轮询，不自动上传未确认的新文件或二进制内容。
